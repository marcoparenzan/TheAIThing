...
public class VirtualSignalsService([ServiceKey] string serviceKey, IConfiguration config, IServiceProvider sp, ILogger<VirtualSignalsService> logger) : IWorkerService
{
    ...
    {
        var workerConfig = config.GetSection("services").GetSection(serviceKey);
        var filename = workerConfig[$"fileName"];
